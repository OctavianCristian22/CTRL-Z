...
import UplinkProfile from './pages/UPLINK/UplinkProfile';
import ApplyMentor from './pages/UPLINK/ApplyMentor';
import UplinkAdmin from './pages/UPLINK/UplinkAdmin';
import AboutNeural from './pages/UPLINK/AboutNeural';
import FAQNeural from './pages/UPLINK/FAQNeural';
import AboutHub from './pages/HUB/AboutHub';
import FAQHub from './pages/HUB/FAQHub';
import NeuralUplink from './pages/UPLINK/NeuralUplink';
...
export default function App() {
    ...
    const down = (e) => {
      ...
      if (e.key === konamiCode[konamiIndex]) {
        ...
        if (nextIndex === konamiCode.length) {
            setHackedMode(prev => !prev);
            toast("SYSTEM_OVERRIDE_EXECUTED", { icon: '💀', style: { background: 'red', color: 'white' } });
            setKonamiIndex(0);
        } else {
  ...
  useEffect(() => {
    if (!showLoginModal) {
      const timer = setTimeout(() => {
        setIsRegistering(false); setVerificationSent(false); setEmail(''); setPassword(''); setUsername(''); setPhone(''); setError(''); setPinInput('');
      }, 200);
  ...
  const handleCheckout = async (checkoutData = {}) => {
    ...
    if (!user) { toast.error("Trebuie să fii logat!"); setShowLoginModal(true); return; }
    if (cart.length === 0) { toast.error("Coșul e gol, hackerule!"); return; }
    const toastId = toast.loading("Procesare tranzacție...");
    try {
      const orderTotal = finalTotal !== undefined ? finalTotal : cart.reduce((acc, item) => acc + (Number(item.price) * (item.quantity || 1)), 0);
      const orderData = { userId: user.uid, userEmail: user.email, username: userData?.username || user.displayName || 'Anon', phone: userData?.phone || 'N/A', items: cart, total: Number(orderTotal.toFixed(2)), discountApplied: discountValue || 0, status: 'pending', createdAt: serverTimestamp() };
      await addDoc(collection(db, "orders"), orderData);
      const batch = writeBatch(db);
      cart.forEach((item) => { const productRef = doc(db, "products", item.id); batch.update(productRef, { stock: increment(-item.quantity) }); });
      await batch.commit();
      toast.success(`Comandă acceptată!`, { id: toastId });
      setCart([]); setIsCartOpen(false);
    } catch (error) { console.error("Checkout Error:", error); toast.error("Eroare server. Încearcă iar.", { id: toastId }); }
  };
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (!userSnap.exists()) {
        await setDoc(userDocRef, { username: user.displayName || "User Nou", email: user.email, phone: "", createdAt: serverTimestamp(), twoFactorEnabled: false, photoURL: user.photoURL, role: "user" });
        toast.success("Cont Google configurat! Bine ai venit.", { icon: '👋' });
      } else { toast.success("Te-ai logat cu Google.", { icon: '🔓' }); }
      setShowLoginModal(false);
    } catch (e) { if (e.code === 'auth/popup-closed-by-user') toast('Ai anulat logarea.', { icon: '❌' }); else toast.error(`Eroare: ${e.message}`); }
  };
  ...
  const handleEmailAuth = async (e) => {
    e.preventDefault(); setError(''); setVerificationSent(false);
    try {
      if (isRegistering) {
        ...
        await logout(); setVerificationSent(true); setIsRegistering(false);
      } else {
        if (rememberMe) await setPersistence(auth, browserLocalPersistence); else await setPersistence(auth, browserSessionPersistence);
  ...
  const CommandPalette = () => {
    ...
    const actions = [
        { label: 'Go to Hub', action: () => navigate('/hub') },
        ...
        { label: 'Neural Uplink', action: () => navigate('/uplink') }
    ];
    return (
    ...
    <div className={`min-h-screen font-mono relative overflow-x-hidden ${isImmersive ? 'bg-black p-0' : 'bg-concrete p-0 md:p-8 md:pt-0 bg-noise'}`}>
        ...
        <nav className="flex flex-col md:flex-row justify-between items-center mb-16 border-b-4 border-black pb-6 sticky top-0 bg-[#f0f0f0] z-50 p-4 md:pt-4 md:ml-16 transition-all">
          ...
          <div className="flex items-center gap-4 mt-4 md:mt-0">
            ...
            <button onClick={() => { if (!user) setShowLoginModal(true); else setIsCartOpen(true); }} className="flex items-center gap-2 font-bold hover:bg-neon hover:text-black p-2 border-2 border-transparent hover:border-black transition-all relative">
              <div className={cart.length > 0 ? 'animate-bump' : ''}><ShoppingCart /></div><span className="hidden md:inline">LOOT ({cart.length})</span><span className="md:hidden">({cart.length})</span>
      ...
      <main className={`min-h-[50vh] ${!isImmersive ? 'md:ml-16' : ''}`}>
        <Routes>
          ...
          <Route path="/hub" element={<Hub user={user} userData={userData} openLoginModal={() => setShowLoginModal(true)} />} />
          <Route path="/store" element={<Home />} />
          <Route path="/shop" element={<Shop addToCart={addToCart} user={user} setCart={setCart} />} />
          <Route path="/product/:id" element={<ProductPage addToCart={addToCart} user={user} />} />
          <Route path="/about" element={<About />} />
          <Route path="/faq" element={<FAQ />} />
          <Route path="/profile" element={user ? <Profile user={user} userData={userData} setUser={setUser} setUserData={setUserData} setCart={setCart}/> : <Navigate to="/" />} />
          ...
          <Route path="/uplink" element={<NeuralUplink user={user} userData={userData} openLoginModal={() => setShowLoginModal(true)} /> } />
          <Route path="/uplink/profile" element={user ? <UplinkProfile user={user} userData={userData} /> : <Navigate to="/" />} />
          <Route path="/uplink/apply" element={<ApplyMentor user={user} userData={userData} />} />
          <Route path="/uplink/about" element={<AboutNeural />} />
          <Route path="/uplink/faq" element={<FAQNeural />} />
          <Route path="/uplink/admin" element={<UplinkAdmin userData={userData} />} />
          <Route path="/hub/AboutHub" element={<AboutHub />} />
          <Route path="/hub/FAQHub" element={<FAQHub />} />
