...
import { signInWithGoogle, logout, auth, registerWithEmail, loginWithEmail, updateUserProfile, createUserDocument, getUserDocument, checkUsernameAvailability } from './firebase';
...
export default function App() {
  ...
  const handleGoogleLogin = async () => { try { await signInWithGoogle(); } catch (e) { toast.error("Eroare Google"); } };
  ...
  const handleEmailAuth = async (e) => {
    ...
    try {
      if (isRegistering) {
        const phoneRegex = /^07[0-9]{8}$/; if (!phoneRegex.test(phone)) throw new Error("Telefon invalid.");
        ...
        await updateUserProfile(newUser, username); await createUserDocument(newUser, { username, phone, twoFactorEnabled: false });
  ...
  const CommandPalette = () => {
      ...
      const actions = [ { label: 'Go Home', action: () => navigate('/') }, { label: 'Shop Gear', action: () => navigate('/shop') }, { label: 'View Profile', action: () => navigate('/profile') }, { label: 'View Loot', action: () => setIsCartOpen(true) } ];
      return (
  ...
  return (
    <div className="min-h-screen font-mono bg-concrete p-0 md:p-8 md:pt-0 bg-noise relative overflow-x-hidden">
      ...
      <CartDrawer isOpen={isCartOpen} onClose={() => setIsCartOpen(false)} cart={cart} setCart={setCart} user={user} userData={userData} openLoginModal={() => setShowLoginModal(true)} />
      ...
      <footer className="text-center py-10 border-t-4 border-black opacity-50 hover:opacity-100 transition-opacity mt-20">
