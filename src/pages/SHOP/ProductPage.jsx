...
import { ShoppingCart, Star, ArrowLeft, Pencil, Trash2, X, Save, Heart } from 'lucide-react';
    ...
    <div className="min-h-screen bg-concrete font-mono pb-20 pt-24">
      ...
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <div className="bg-white border-4 border-black p-4 shadow-brutal sticky top-24 h-fit">
            {/* IMAGINEA PRINCIPALA (MARE) */}
            <div className="aspect-square bg-gray-100 overflow-hidden border-2 border-black relative group">
                 {activeImage ? (
                    <img 
                        src={activeImage} 
                        alt={product.name}
                    ...
                    <div className="w-full h-full flex items-center justify-center font-black text-4xl text-gray-300">NO SIGNAL</div>
                 )}
                 
                 {/* Categoria */}
                 <div className="absolute top-4 left-4 bg-black text-neon px-3 py-1 font-bold border border-neon shadow-[4px_4px_0_0_#39FF14]">
                    {product.category?.toUpperCase() || 'ITEM'}
                 </div>

                 {/* Indicator Low Stock pe imagine */}
                 {product.stock !== undefined && product.stock > 0 && product.stock < 5 && (
                    <div className="absolute bottom-4 right-4 bg-red-600 text-white px-2 py-1 font-bold border-2 border-black animate-pulse text-xs">
                ...
                <div className="flex gap-2 mt-4 overflow-x-auto pb-2">
                    {product.images.map((img, idx) => (
                        <button 
                            ...
                            className={`w-20 h-20 flex-shrink-0 border-2 transition-all cursor-pointer ${
                                activeImage === img 
                                ? 'border-neon ring-2 ring-black opacity-100' 
        ...
        <div className="flex flex-col gap-6">
            <div>
                ...
                <div className="flex items-center gap-2 mb-4">
                    <div className="flex text-neon bg-black px-2 py-1">
                        {[1,2,3,4,5].map(star => (
                            <Star key={star} size={16} fill={star <= Math.round(Number(averageRating)) ? "#39FF14" : "none"} className={star <= Math.round(Number(averageRating)) ? "text-neon" : "text-gray-600"} />
                        ))}
                    </div>
                ...
                {/* PRICE */}
                <div className="text-4xl font-black text-black mb-6 flex items-center gap-4">
                    {product.price} RON
                    {product.stock === 0 && <span className="text-red-600 text-lg border-2 border-red-600 px-2 -rotate-6 uppercase">Sold Out</span>}
                </div>
            ...
            <div className="bg-white border-4 border-black p-6 shadow-[8px_8px_0_0_rgba(0,0,0,1)]">
                <div className="flex gap-4 mb-4">
                    ...
                    <div className="flex items-center border-2 border-black bg-gray-100">
                        <button onClick={() => setQty(q => Math.max(1, q - 1))} className="px-4 py-2 hover:bg-black hover:text-white font-black text-xl" disabled={product.stock === 0}>-</button>
                        <span className="px-4 font-bold text-xl min-w-[50px] text-center">{qty}</span>
                        <button onClick={() => setQty(q => q + 1)} className="px-4 py-2 hover:bg-black hover:text-white font-black text-xl" disabled={product.stock === 0}>+</button>
                    </div>
                    ...
                    <button 
                        ...
                        className={`flex-1 font-black text-xl uppercase tracking-widest border-2 border-black transition-all flex items-center justify-center gap-2 ${
                            product.stock === 0 
                            ? 'bg-gray-300 text-gray-500 cursor-not-allowed' 
                            : 'bg-neon text-black hover:bg-black hover:text-neon'
                        }`}
                    >
                        <ShoppingCart /> 
                        <span className="hidden md:inline">{product.stock === 0 ? 'OUT OF STOCK' : 'ADD TO LOOT'}</span>
                        <span className="md:hidden">{product.stock === 0 ? 'SOLD' : 'ADD'}</span>
                    </button>
                                ...
                                {editingId === rev.id ? (
                                    ...
                                    <>
                                        <div className="flex justify-between items-start mb-2">
                                            <span className="font-black uppercase">{rev.userName}</span>
                                            <div className="flex">{[...Array(5)].map((_, i) => <Star key={i} size={12} fill={i < rev.rating ? "black" : "none"} />)}</div>
                                        </div>
                                        <p className="text-sm font-bold text-gray-600">{rev.text}</p>
                                        <div className="flex justify-between items-end mt-3">
                                            <span className="text-[10px] text-gray-400 uppercase block">{rev.isEdited && <span className="text-neon mr-2">(EDITED)</span>} VERIFIED</span>
                                            {user && user.uid === rev.userId && (
                                                <div className="flex gap-2">
                                                    <button onClick={() => startEditing(rev)} className="text-gray-500 hover:text-blue-600"><Pencil size={14}/></button>
                                                    <button onClick={() => handleDeleteReview(rev.id)} className="text-gray-500 hover:text-error"><Trash2 size={14}/></button>
                                                </div>
                                            )}
                                        </div>
                                    </>
                ...
                <div className="bg-black p-6 text-white border-4 border-neon shadow-[8px_8px_0_0_#39FF14] h-fit sticky top-24">
                    ...
                    {!user ? (
                        <div className="text-center py-8">
                            <p className="mb-4 font-bold">ACCESS DENIED.</p>
                            <Link to="/shop" className="text-neon underline">Login Required</Link>
                        </div>
                    ) : (
                                ...
                                <div className="flex gap-2">
                                    {[1,2,3,4,5].map(star => (
                                        <button type="button" key={star} onClick={() => setNewRating(star)} className="focus:outline-none">
                                            <Star size={24} fill={star <= newRating ? "#39FF14" : "none"} className={star <= newRating ? "text-neon" : "text-gray-600"} />
                                        </button>
                                    ))}
                                </div>
                            </div>
                            <textarea rows="4" className="w-full bg-gray-900 border-2 border-gray-700 p-2 text-white focus:border-neon focus:outline-none font-mono" placeholder="System log entry..." value={newReviewText} onChange={e => setNewReviewText(e.target.value)}></textarea>
                            <button type="submit" className="w-full bg-neon text-black font-black py-3 uppercase tracking-widest hover:bg-white border-2 border-transparent hover:border-black transition-all">UPLOAD</button>
                        </form>
                    )}
                </div>
            </div>
